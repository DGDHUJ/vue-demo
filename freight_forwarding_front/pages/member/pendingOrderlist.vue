        ...
        <div class="list-content">
          <div class="items">
            <el-row v-for="(item,index,key) in listdata.list" :key="key">
              ...
              <el-col :span="4"><div class="grid-content bg-purple">
                <div v-if="item.status ==='wait_confirm_offer'">客服已回复准备报价</div>
                <div v-if="item.status ==='wait_confirm_offer'" class="look">等待您的确认</div>
                <div v-if="item.status ==='wait_offer'">正在等待客服回复</div>
                <div v-if="item.status ==='wait_offer'" class="look">请稍等</div>
              </div></el-col>
              <el-col :span="4"><div class="grid-content bg-purple">
                ￥{{item.total_price}}
              </div></el-col>
              <el-col :span="4"><div class="grid-content bg-purple look" v-if="item.status ==='wait_confirm_offer'" @click="check(item.id,item.sn)">
                查看
              </div></el-col>
            </el-row>
          </div>
