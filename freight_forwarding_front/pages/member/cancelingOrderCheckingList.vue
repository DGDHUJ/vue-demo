        ...
        <div class="list-content">
          <div class="items">
            <el-row v-for="(item,index,key) in listdata.list" :key="key">
              ...
              <el-col :span="4"><div class="grid-content bg-purple">
               <span v-if="item.status === 'check_for_cancel'">审核中...</span>
               <span  v-if="item.status === 'cancel_wait_confirm'">审核完，待确定取消</span>
              </div></el-col>
              <el-col :span="4"><div class="grid-content bg-purple">
                ￥{{item.total_price}}
              </div></el-col>
              <el-col v-if="item.status === 'cancel_wait_confirm'" :span="4"><div class="grid-content bg-purple look" @click="check(item.id)">
                查看
              </div></el-col>
            </el-row>
          </div>
...
<script>
  export default {
    name:'cancelingOrderCheckingList',
    data(){
      return{
        listdata:[],
        currentpage:1,
